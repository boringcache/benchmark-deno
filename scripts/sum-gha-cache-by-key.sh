...
#
# For non-Docker benchmarks (e.g., linkerd2-proxy-sccache or grpc-bazel), sums entries matching the key prefix.
...
is_docker_benchmark() {
  ...
  # Non-docker: linkerd2-proxy-sccache, grpc-bazel, etc.
  case "$prefix" in
...
# Sum entries matching the benchmark key prefix (e.g., linkerd2-proxy-sccache-*)
prefix_sum="$(
  sum_for_query \
    "${benchmark_or_prefix}" \
    "${timestamp_filter}
