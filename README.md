...
The [sccache-only comparison](.github/workflows/deno-runs-on-sccache-only.yml) uses the same builds and output checks, but neither provider restores `target` on a fresh runner. Both retain Cargo registry and Git dependencies; BoringCache uses remote sccache, and Magic Cache archives a local sccache directory.
